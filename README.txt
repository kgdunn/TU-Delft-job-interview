This demonstration code was written and tested in XCode 7.3. It has not yet been tested on other platforms.
Nothing in the code has been used that would necessarily prevent it from working on other platforms.

To compile and run the code you will require 5 other libraries. Details for installing and testing these are given below.


1. Download FFTW: http://www.fftw.org/download.html
1a) ./configure
1b) make
1c) make install (installs the library to /usr/local/include)
1d) Update the XCode settings for the project to include the above directory path: Build Settings --> Search Paths --> "Header Search Paths"
1e) Now you have use it in the code: #include <fftw3.h>
1f) You also need to adjust the "Other Linker Flags" setting: -lfftw3 -lm
1g) And you need to adjust where the library files (the *.a files) are: Build Settings --> Search Paths --> "Library Search Paths" and add "/usr/local/lib"

2. BMP library: http://partow.net/programming/bitmap/
2a) download the files and add them to the C++ project

3. Download OpenCV: http://opencv.org/downloads.html
...
3d. The last part of this directory will change: /Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX10.11.sdk/
...
3h. Pay attention to where the .hpp files are copied. For example: "/usr/local/include/"
...
5. Boost: 
5a. Download the latest version: http://www.boost.org/doc/libs/1_60_0/more/getting_started/unix-variants.html
5b. Unpack it: tar -zxfv boost_1_60_0.tar.bz2
5c. ./bootstrap.sh
5d. ./b2
5e. ./b2 install
5f. As with OpenCV, you will need to update your search paths for the Library and Header files respectively, but likely
